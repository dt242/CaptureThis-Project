...
class GlobalModelAttributesTest {
    ...
    @Test
    void testAddGlobalAttributes_whenUserIsNotLoggedIn() {
        try (MockedStatic<SecurityUtil> mockSecurityUtil = mockStatic(SecurityUtil.class)) {
            mockSecurityUtil.when(SecurityUtil::getSessionUser).thenReturn(null);

            globalModelAttributes.addGlobalAttributes(model);

            verifyNoInteractions(userService, notificationService, model);
        }
    }
}